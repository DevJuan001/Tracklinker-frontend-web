      ...
      <section className="order-2 md:py-3.5">
        <ul className="flex flex-col gap-1">
          ...
          {secondSectionItems.map((item) => (
            <li
              key={item.name}
              className="rounded-2xl transition-colors duration-300 text-[#75777E] font-normal
              hover:bg-[#e5e7eb96]
            ...
            >
              {item.path ? (
                ...
                <button onClick={helpOnClick} className="w-full">
                  <section
                    className="w-full h-14 flex items-center justify-center gap-2.5 py-2.5 subpixel-antialiased transition-colors duration-300 group
                    md:pl-0
